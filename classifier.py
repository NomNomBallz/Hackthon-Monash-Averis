...
class FinalClassifierPipeline:
    ...
    def local_pre_filter(self, email_data):
        ...
        suspicious_words = ["PRIZE", "PARCEL", "PHISHING", "WINNER",
                            "CLICK HERE", "LOGIN", "URGENT", "ACCOUNT SUSPENDED"]
        ...
        if any(phrase in subject for phrase in ["TO CONFIRM DOCS", "REQUEST BL DRAFT", "DRAFT BL", "AIE - ", "AFEMY - ", "AFRT - ", "AFPTME - "]):
            return "BL_COMPARISON"
        ...
        for attempt in range(max_retries):
            try:
                ...
                if "429" in error_message or "RESOURCE_EXHAUSTED" in error_message or "503" in error_message or "UNAVAILABLE" in error_message:
                    print(
                        f"   [Server/Quota] Pausing for 10 seconds (Attempt {attempt + 1}/{max_retries})...")
                    time.sleep(10)
                    continue
                return {}
        return {}
    ...
    def process_inbox(self):
        ...
        print(
            f"Starting Smart Classification for {len(email_files)} emails...\n")
        ...
        print(
            f"⚡ Python instantly verified and categorized {len(email_files) - len(ai_queue)} emails.")
        ...
        for i in range(0, total_ai_files, self.batch_size):
            ...
            print(
                f"Processing AI batch {i//self.batch_size + 1} of {(total_ai_files + self.batch_size - 1)//self.batch_size}...")
    ...
    def export_for_member_3(self, classified_emails, output_file="filtered_bl_comparison.json"):
        ...
        print(f"\n==================================================")
        print(
            f"✅ Handoff Ready! Exported {len(bl_emails)} BL_COMPARISON emails to {output_file}")
        print(f"==================================================")
